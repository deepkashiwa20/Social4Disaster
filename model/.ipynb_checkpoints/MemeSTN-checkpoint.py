...
def main():
    ...
    CHANNELIN,CHANNELOUT,N_NODE,TIMESTEP_IN,TIMESTEP_OUT,T_DIM = 1, 1, 47, 6, 6, 32
    GPU = sys.argv[-1] if len(sys.argv) == 2 else '3'
    device = torch.device("cuda:{}".format(GPU)) if torch.cuda.is_available() else torch.device("cpu")
    model = MemeSTN(num_nodes=N_NODE, input_dim=CHANNELIN, output_dim=CHANNELOUT, horizon=TIMESTEP_OUT).to(device)
